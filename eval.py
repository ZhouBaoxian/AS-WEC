...
def cal_miou(test_dir="data/Test_Images",
    ...
    # 取出空的字符串
    image_ids = [x for x in image_ids if x.strip()]
    # 加载模型
    if miou_mode == 0 or miou_mode == 1:
        ...
        # 加载模型参数
        net.load_state_dict(torch.load('weights/best_model_SegNet.pth', map_location=device)) # todo
        # 测试模式
        net.eval()
    ...
    if miou_mode == 0 or miou_mode == 2:
        ...
        print(num_classes)
        print(name_classes)
        hist, IoUs, PA_Recall, Precision, Dice, Sen, MAE = compute_mIoU(gt_dir, pred_dir, image_ids, num_classes,
