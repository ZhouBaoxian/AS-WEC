...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):
    ...
    for ind in range(num_img):
        ...
        false_negative = np.logical_and(pred, np.logical_not(label)).sum()
    ...
    print('===> mIoU: ' + str(round(np.nanmean(IoUs) * 100, 2)) + '; mPA: ' + str(
        round(np.nanmean(PA_Recall) * 100, 2)) + '; Accuracy: ' + str(round(per_Accuracy(hist) * 100, 2))
          + '; Dice: ' + str(round(Dice * 100, 2)) + '; Sen: ' + str(round(Sen * 100, 2)) + '; MAE: ' + str(round(MAE * 100, 2)))
    return np.array(hist, int), IoUs, PA_Recall, Precision, Dice, Sen, MAE
